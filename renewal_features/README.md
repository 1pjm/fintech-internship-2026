...
- **광주/전남은 학습기간(2022~2024) 전체에서 실거래가 API 응답이 0건** (대표 시군구를 광산구/여수시로
  바꿔봐도 동일, 2022-01/2023-12/2024-06 세 시점 모두 확인함). 2026-07-01 광주-전남
  행정통합("전남광주통합특별시") 이후 법정동코드 체계가 개편되면서 이 API가 구코드로는 데이터를
  못 찾는 것으로 추정된다. 두 지역은 `갱신피처_출처`가 `national`(전국평균 대체)로만 채워진다.
